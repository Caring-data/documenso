...
This will start the server on `localhost:3002`. Any reverse proxy can handle the front end and SSL termination.
...
Access the Documenso application by visiting `http://localhost:3002` in your web browser.
...
| -------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                       | The port on which the Documenso application runs. It defaults to `3002`.                            |
...
You can run the application using a `systemd.service` file. Here is a simple example of the service running on port `3500` (using `3002` by default):
