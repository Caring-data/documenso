...
import { getSignerColorStyles, useSignerColors } from '@documenso/ui/lib/signer-colors';
...
export const AddTemplateFieldsFormPartial = ({
  ...
  const onFieldCopy = useCallback(
    ...
    [append, lastActiveField, selectedSigner, toast],
  );
  ...
  const filterFieldsWithEmptyValues = useCallback(
    (fields: typeof localFields, fieldType: string) =>
      fields
  ...
  const recipientsByRoleToDisplay = useMemo(() => {
    return Object.entries(recipientsByRole).filter(
      ([role]) =>
                  ...
                  <PopoverContent className="p-0" align="start">
                    <Command value={selectedSigner?.email ?? undefined}>
                      ...
                      {recipientsByRoleToDisplay.map(([role, roleRecipients], roleIndex) => {
                        const recipientRole = role as RecipientRole;
                        return (
                          <CommandGroup key={roleIndex}>
                            <div className="text-muted-foreground mb-1 ml-2 mt-2 text-xs font-medium">
                              {_(RECIPIENT_ROLES_DESCRIPTION[recipientRole].roleNamePlural)}
                            </div>
                ...
                <div className="mt-4 flex flex-col gap-2">
                  ...
                  <Accordion type="single" collapsible>
                    <AccordionItem value="resident-fields">
                      ...
                      <AccordionContent>
                        <fieldset className="-mx-2 flex-1 overflow-y-auto px-2">
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_FIRST_NAME)}
                              onMouseDown={() => setSelectedField(FieldType.RESIDENT_FIRST_NAME)}
                              data-selected={
                                selectedField === FieldType.RESIDENT_FIRST_NAME ? true : undefined
                              }
                            >
                              <Card
                                className={cn(
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_LAST_NAME)}
                              onMouseDown={() => setSelectedField(FieldType.RESIDENT_LAST_NAME)}
                              data-selected={
                                selectedField === FieldType.RESIDENT_LAST_NAME ? true : undefined
                              }
                            >
                              <Card
                                className={cn(
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_DOB)}
                              onMouseDown={() => setSelectedField(FieldType.RESIDENT_DOB)}
                              data-selected={
                                selectedField === FieldType.RESIDENT_DOB ? true : undefined
                              }
                            >
                              <Card
                                className={cn(
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_GENDER_IDENTITY)}
                              onMouseDown={() =>
                                setSelectedField(FieldType.RESIDENT_GENDER_IDENTITY)
                              }
                              data-selected={
                                selectedField === FieldType.RESIDENT_GENDER_IDENTITY
                                  ? true
                                  : undefined
                              }
                            >
                        ...
                        <fieldset className="-mx-2 flex-1 overflow-y-auto px-2">
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_LOCATION_NAME)}
                              onMouseDown={() => setSelectedField(FieldType.RESIDENT_LOCATION_NAME)}
                              data-selected={
                                selectedField === FieldType.RESIDENT_LOCATION_NAME
                                  ? true
                                  : undefined
                              }
                            >
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_LOCATION_STATE)}
                              onMouseDown={() =>
                                setSelectedField(FieldType.RESIDENT_LOCATION_STATE)
                              }
                              data-selected={
                                selectedField === FieldType.RESIDENT_LOCATION_STATE
                                  ? true
                                  : undefined
                              }
                            >
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_LOCATION_ADDRESS)}
                              onMouseDown={() =>
                                setSelectedField(FieldType.RESIDENT_LOCATION_ADDRESS)
                              }
                              data-selected={
                                selectedField === FieldType.RESIDENT_LOCATION_ADDRESS
                                  ? true
                                  : undefined
                              }
                            >
                            ...
                            <button
                              type="button"
                              className="group h-full w-full"
                              onClick={() => setSelectedField(FieldType.RESIDENT_LOCATION_CITY)}
                              onMouseDown={() => setSelectedField(FieldType.RESIDENT_LOCATION_CITY)}
                              data-selected={
                                selectedField === FieldType.RESIDENT_LOCATION_CITY
                                  ? true
                                  : undefined
                              }
                            >
                            ...
                            <button
                              ...
                              data-selected={
                                selectedField === FieldType.RESIDENT_LOCATION_ZIP_CODE
                                  ? true
                                  : undefined
                              }
                            >
